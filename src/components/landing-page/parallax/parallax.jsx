...
const Parallax = ({img, parallaxData, showButton, buttonText, buttonBorderColor, buttonBorderSize, buttonBolded, height, backgroundPosition}) => {
    return (
        <header className='parallax' style={{backgroundImage: `radial-gradient(circle at ${parallaxData.gradientRadius}, rgba(0,0,0,0), rgba(0,0,0,${parallaxData.gradientIntensity})), url(${img})`, height: height, backgroundPosition: backgroundPosition}}>
            ...
            {showButton ? <Link to='/app'><Button text={buttonText} borderColor={buttonBorderColor} borderSize={buttonBorderSize} buttonBolded={buttonBolded} /></Link> : null}
        </header>
    )
}
