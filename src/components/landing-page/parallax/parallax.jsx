...
const Parallax = ({dynamicTitle, linkTo, img, parallaxData, showButton, buttonText, buttonBorderColor, buttonBorderSize, buttonBolded, height, backgroundPosition}) => {
    return (
        <header className='parallax' style={{backgroundImage: `radial-gradient(circle at ${parallaxData.gradientRadius}, rgba(0,0,0,0), rgba(0,0,0,${parallaxData.gradientIntensity})), url(${img})`, height: height, backgroundPosition: backgroundPosition}}>
            <h1 style={{color: parallaxData.color}} className='parallax-title'>{dynamicTitle ? dynamicTitle.slice(0, 33) : parallaxData.title}</h1>
